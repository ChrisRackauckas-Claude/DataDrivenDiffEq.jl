...
@inline function active_set!(
        idx::BitArray, ::SoftThreshold, x::AbstractArray{T},
        λ::T
    ) where {T}
...
@inline function (s::SoftThreshold)(
        y::AbstractArray{N}, x::AbstractArray{N},
        λ::T
    ) where {N <: Number, T <: Real}
    @assert size(y) == size(x)
    for i in eachindex(x)
...
@inline function active_set!(
        idx::BitArray, ::HardThreshold, x::AbstractArray,
        λ::T
    ) where {T}
    @assert size(idx) == size(x)
    @inbounds foreach(eachindex(x)) do i
...
@inline function (s::HardThreshold)(
        y::AbstractArray{N}, x::AbstractArray{N},
        λ::T
    ) where {N <: Number, T <: Real}
...
@inline function active_set!(
        idx::BitArray, h::ClippedAbsoluteDeviation,
        x::AbstractArray, λ::T
    ) where {T}
    @assert size(idx) == size(x)
    @unpack ρ = h
...
function (s::ClippedAbsoluteDeviation)(
        y::AbstractArray{N}, x::AbstractArray{N},
        λ::T
    ) where {N <: Number, T <: Real}
    @assert all(size(y) .== size(x))
